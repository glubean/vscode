...
describe("normalizeFilterId", () => {
  it("strips each: prefix and template variables", () => {
    assert.equal(normalizeFilterId("each:get-user-$id"), "get-user-");
  });

  it("strips pick: prefix and template variables", () => {
    assert.equal(normalizeFilterId("pick:search-$_pick"), "search-");
  });

  it("strips multiple template variables", () => {
    assert.equal(
      normalizeFilterId("each:item-$index-$name"),
      "item--",
    );
  });

  it("returns plain IDs unchanged", () => {
    assert.equal(normalizeFilterId("health-check"), "health-check");
  });

  it("returns empty string for each: prefix with only a template var", () => {
    assert.equal(normalizeFilterId("each:$id"), "");
  });
...
describe("tracePairToCurl", () => {
  ...
  it("escapes single quotes in URL", () => {
    ...
    assert.ok(result.includes("O'\\''Reilly"));
    assert.ok(!result.match(/O'Reilly'/));
  });
...
function entry(testId: string, success = true): ResultTestEntry {
  return { testId, testName: testId, success, durationMs: 10, events: [] };
}

describe("matchTestResults", () => {
  it("exact match for plain test IDs", () => {
    const ids = ["health", "smoke"];
    const results = [entry("smoke"), entry("health")];
    const matched = matchTestResults(ids, results);

    assert.equal(matched[0].length, 1);
    assert.equal(matched[0][0].testId, "health");
    assert.equal(matched[1].length, 1);
    assert.equal(matched[1][0].testId, "smoke");
  });

  it("prefix match for each: data-driven tests", () => {
    const ids = ["each:get-user-$id"];
    const results = [entry("get-user-1"), entry("get-user-2"), entry("other")];
    const matched = matchTestResults(ids, results);
  ...
  it("prefix match for pick: data-driven tests", () => {
    const ids = ["pick:search-products-$_pick"];
    const results = [
      entry("search-products-by-name"),
      entry("search-products-by-price"),
    ];
    const matched = matchTestResults(ids, results);
  ...
  it("empty prefix gets only unclaimed results", () => {
    const ids = ["health", "each:$id"];
    const results = [entry("1"), entry("2"), entry("health")];
    const matched = matchTestResults(ids, results);

    assert.equal(matched[0].length, 1);
    assert.equal(matched[0][0].testId, "health");
    assert.equal(matched[1].length, 2);
    assert.deepEqual(
      matched[1].map((r) => r.testId),
      ["1", "2"],
    );
  });

  it("empty prefix does not steal from prefix-matched tests", () => {
    const ids = ["each:get-user-$id", "each:$name"];
    const results = [
      entry("get-user-1"),
      entry("get-user-2"),
      entry("alice"),
  ...
  it("empty prefix does not steal from exact-matched tests", () => {
    const ids = ["each:$x", "health"];
    const results = [entry("health"), entry("100"), entry("200")];
    const matched = matchTestResults(ids, results);
    ...
    assert.equal(matched[1].length, 1);
    assert.equal(matched[1][0].testId, "health");
  });

  it("unmatched plain test ID yields empty array", () => {
    const ids = ["nonexistent"];
    const results = [entry("health")];
    const matched = matchTestResults(ids, results);

    assert.equal(matched[0].length, 0);
  });

  it("unmatched each: prefix yields empty array", () => {
    const ids = ["each:get-user-$id"];
    const results = [entry("list-items-1")];
    const matched = matchTestResults(ids, results);

    assert.equal(matched[0].length, 0);
  });

  it("multiple empty-prefix items: first claims all unclaimed", () => {
    const ids = ["each:$a", "each:$b"];
    const results = [entry("x"), entry("y")];
    const matched = matchTestResults(ids, results);
  ...
  it("empty results array yields all empty matches", () => {
    const ids = ["health", "each:get-user-$id"];
    const matched = matchTestResults(ids, []);

    assert.equal(matched[0].length, 0);
    assert.equal(matched[1].length, 0);
  });

  it("empty test IDs array yields empty outer array", () => {
    const matched = matchTestResults([], [entry("health")]);
    assert.equal(matched.length, 0);
  });

  it("mixed: plain + prefix + empty-prefix all resolve correctly", () => {
    const ids = ["health", "each:get-user-$id", "each:$n"];
    const results = [
      entry("get-user-1"),
      entry("health"),
      entry("get-user-2"),
    ...
    assert.equal(matched[2].length, 2);
    assert.deepEqual(
